    ...
    public Mono<AuthServiceResponseDto> createUserInAuthService(UserRequestDto userRequestDto) {
        ...
        return authServiceClient.post()
                ...
                .onStatus(HttpStatusCode::is5xxServerError,
                        response -> Mono.error(new InternalServiceException("Server error")))
                .bodyToMono(AuthServiceResponseDto.class)
                .retryWhen(Retry.backoff(3, Duration.ofSeconds(1))
                        .filter(ex -> ex instanceof InternalServiceException
                                || ex instanceof WebClientRequestException))
                .doOnError(error -> log.error("Authentication service call failed: {}", error.getMessage(), error))
    ...
    public Mono<Void> rollbackUserInAuthService(Long userId) {
        ...
        return authServiceClient.delete()
                ...
                .retryWhen(Retry.backoff(3, Duration.ofSeconds(1))
                        .filter(ex -> ex instanceof InternalServiceException
                                || ex instanceof WebClientRequestException))
                .doOnError(error -> log.error("Authentication service call failed: {}", error.getMessage()))
                .doOnSuccess(unused ->
